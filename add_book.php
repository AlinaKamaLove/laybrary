...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $year = trim($_POST['publication_year']); // Год издания задаётся текстом
    $copies = intval($_POST['total_copies']);

    // Загружаем картинку
    $coverImagePath = "";
    if (isset($_FILES['cover_image']['tmp_name']) && is_uploaded_file($_FILES['cover_image']['tmp_name'])) {
        $uploadDir = "uploads/";
    ...
    try {
        // Вставка данных книги
        $stmt = $pdo->prepare("INSERT INTO books (title, author_id, category_id, isbn, publication_year, total_copies, cover_image_path) 
                               VALUES (:title, :author_id, :category_id, :isbn, :publication_year, :total_copies, :cover_image_path)");
        $stmt->execute([
            'title' => $title,
            'author_id' => $authorId,
        ...
        echo "<small class='error-message'>Ошибка добавления книги: {$e->getMessage()}</small>";
    }
}
?>
<!DOCTYPE html>
...
<div class="container">
    ...
    <form enctype="multipart/form-data" method="post">
        ...
        <div class="form-group">
            ...
            <input type="text" name="publication_year" required /> <!-- Год издания задаётся текстом -->
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="file" name="cover_image" accept="image/*"/> <!-- Возможность загрузки фото -->
        </div>
